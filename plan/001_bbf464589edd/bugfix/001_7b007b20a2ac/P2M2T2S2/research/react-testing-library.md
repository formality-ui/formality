...
test("disabled priority: JSX prop > config > conditions > group", async () => {
  ...
  const config = {
    field: {
      ...
      conditions: [{ when: "otherField", is: "match", disabled: true }],
    },
  };
...
test("async data loading", async () => {
  const mockFetch = vi.fn(() => Promise.resolve({ data: "loaded data" }));
...
test("conditional based on multiple field values", () => {
  ...
  render(
    <Form config={config} record={{ country: "USA", state: "CA" }}>
      <Field name="country" />
      <Field name="state" />
      <Field name="zipCode" />
    </Form>,
...
test("field visibility based on condition", () => {
  ...
  render(
    <Form config={config} record={{ toggle: "yes" }}>
      <Field name="toggle" />
